...
class Items extends Secure_area implements iData_controller
{
	...
	*/
	...
	function view($item_id=-1)
	{
		...
		{
			$category[$row['pharmaceticsClass_id']] = $row['pharmaceticsClass_name'];
		}
		$data['category']=$category;
	
		foreach($this->Supplier->get_all_pharm_forms()->result_array() as $row)
		{
			$form[$row['pharmaceticsFom_name']] = $row['pharmaceticsFom_name'];
		}

		$data['form']=$form;

		$data['selected_supplier'] = $this->Item->get_info($item_id)->supplier_id;
		//$data['selected_category'] = $this->Item->get_info_categories($item_id)->pharmaceticsClass_id;
