...
class Sale_lib
{
	...
	function add_item($item_id,$quantity=1,$item_location,$discount=0,$price=null,$description=null,$serialnumber=null)
	{
		...
		$price=$price!=null?$price:$item_info->retail_price;
		$retprice=$price!=null?$price:$item_info->retail_price;
		...
		if(!$itemalreadyinsale || $item_info->is_serialized)
		{
            $item = array(($insertkey)=>
                array(
                    ...
                    'price'=>$price,
                    'retprice'=>$item_info->retail_price,
                    'total'=>$total,
	...
	function calculate_subtotal($include_discount=FALSE, $exclude_tax=FALSE) 
	{
		...
		foreach($this->get_cart() as $item)
		{
			...
			{
				$subtotal = bcadd($subtotal, $this->get_item_total_tax_exclusive($item['item_id'], $item['quantity'],$price_value, $item['discount'], $include_discount), PRECISION);
			}
			else 
			{
