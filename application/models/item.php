...
class Item extends CI_Model
{
	...
	*/
	public function get_info_categories($item_id)
	{
		// $this->db->select('items.*');
		// $this->db->select('pharm_aceticsclass.pharmaceticsClass_name');
		 $this->db->from('pharm_aceticsclass');
		//$this->db->join('pharm_aceticsclass', 'suppliers.person_id = items.supplier_id', 'left');
		//$this->db->where('item_id', $item_id);
		...
		else
		{
			...
			//Get all the fields from items table
			$fields = $this->db->list_fields('pharm_aceticsclass');
	...
	*/
	...
	public function get_item_search_suggestions($search, $limit=25, $search_custom=0, $is_deleted=0)
	{
		...
		$this->db->select('item_id, name, wholesale_price');
		$this->db->from('items');
		$this->db->where('deleted', $is_deleted);
		$this->db->like('name', $search);
		$this->db->order_by('name', 'asc');
		...
		//Search by description
		$this->db->select('item_id, name, description, wholesale_price');
		$this->db->from('items');
		$this->db->where('deleted', $is_deleted);
		$this->db->like('description', $search);
		$this->db->order_by('description', 'asc');
