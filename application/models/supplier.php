...
/* return all categories from the system */
		function get_all_categories($limit_from = 0, $rows = 0)
	{
...
/* return all categories from the system */ 
		function get_all_pharm_forms($limit_from = 0, $rows = 0)
	{
		$this->db->from('pharm_aceticsfom');			
		//$this->db->where('deleted', 0);
		if ($rows > 0) {
			$this->db->limit($rows, $limit_from);
		}
